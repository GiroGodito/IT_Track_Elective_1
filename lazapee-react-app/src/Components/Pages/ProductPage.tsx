...
//LAST WORKING CODE IMPLEMENTATION STARTS HEREE AHHHHHH
...
export default function ProductPage() {
    ...
    // ✅ Hydrate cart state from backend
    useEffect(() => {
        if (isLoggedIn) {
            hydrateCartState();
        }
    ...
    const hydrateCartState = async () => {
        try {
            const data = await cartService.viewCart({ currentPage: 1, pageSize: 1000 });
            const carts = data.cartList;
    ...
    useEffect(() => {
        productService
            .getProducts({ currentPage, pageSize: productsPerPage })
            .then((data) => {
    ...
    const handleQuantityChange = async (productID: number, value: number) => {
        ...
        if (cartItemID && cartID) {
            try {
                await cartItemService.updateCartItem(cartItemID, {
                    cartItemID,
                    cartID,
                    productID,
                    quantity: value,
    ...
    const AddToCart = async (sellerID: number, productID: number, quantity: number) => {
        ...
        try {
            ...
            if (!cartID) {
                cartID = await cartService.createCart({ sellerID });
                setCartIDs((prev) => ({ ...prev, [sellerID]: cartID }));
            }
            ...
            if (cartItemID) {
                ...
                const newCartItemID = await cartItemService.createCartItem({
                    cartID,
                    productID,
                    quantity,
                });
                setCartItemIDs((prev) => ({ ...prev, [productID]: newCartItemID }));
            }
